...
#Separating long format pH data from original data set
pH_long <- df |> 
  dplyr::select(
    Treatment = `Treatment_pH`,
    Replicate,
    Day1_pH,
    Day5_pH,
    Day10_pH
  ...
  pivot_longer(
    cols = c(Day1_pH, Day5_pH, Day10_pH),
    names_to = "Day",
    values_to = "pH"
  ) |> 
  mutate(
    Day = str_remove(Day, "_pH"),
...
pH_anova <- aov(pH ~ Treatment * Day, data = pH_long)
summary(pH_anova)
# Checking assumptions of the annova for pH
shapiro.test(residuals(pH_anova))
# residual plots ( residuals vs fitted) 
par(mfrow = c(1, 2), mar = c(4, 4, 2, 1))
qqnorm(residuals(pH_anova))
qqline(residuals(pH_anova))
plot(pH_anova, which = 1)
# Homogeneity of variance 
install.packages("car")
library(car)
leveneTest(pH ~ Treatment * Day, data = pH_long)
#Post-hoc comparisons (from two-way model)
# Treatments within Day (Uppercase letters)
cld_trt <- emmeans(pH_anova, ~ Treatment | Day) |> 
...
# GT table
gt_table_pH <- final_display |> 
  gt() |> 
  tab_header(
    title = md(
      "**Table 9.** Effect of *Piper chaba* extract on pH of fresh chicken patties during storage."
    )
  ) |> 
...
gtsave(gt_table_tvc, filename = "pH_table.docx")

##Visualization of line plot and bar plot of TVC data
# Line plot visualization for pH data
P_pH_line <- ggplot(
  summary_stats,
  aes(
    x = Day,
...
# Save the pH plot (tiff/pdf)
install.packages("extrafont")
library("extrafont")
ggsave(
  filename = "pH_line_plot.tiff",
  plot = P_pH_line,
  device = "tiff",
  width = 14,
  height = 10,
...
p_pH_bar <- ggplot( plot_data,
  ...
  p_pH_bar
# saving the bar plot for pH data(tiff/)
ggsave(
  filename = "pH_bar_plot.tiff",
  plot = p_pH_bar,
  device = "tiff",
  width = 19,
  height = 11,
