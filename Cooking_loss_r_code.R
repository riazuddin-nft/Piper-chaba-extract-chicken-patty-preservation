#Separating long format CL data from original data set
cl_long <- df |> 
  dplyr::select(
    Treatment = `Treatment_CL`,
    Replicate,
    Day1_CL,
    Day5_CL,
    Day10_CL
  ...
  pivot_longer(
    cols = c(Day1_CL, Day5_CL, Day10_CL),
    names_to = "Day",
    values_to = "Cooking_Loss"
  ) |> 
  mutate(
    Day = str_remove(Day, "_CL"),
    Treatment = factor(Treatment, levels = c("T0", "T1", "T2", "T3", "T4")),
    Day = factor(Day, levels = c("Day1", "Day5", "Day10")),
    Replicate = factor(Replicate),
    Cooking_Loss  = as.numeric(Cooking_Loss)
  )
#Checking replicate number per treatment, day
cl_long |> 
  count(Treatment, Day)
#Two way annova (Treatment*Day)
cl_anova <- aov(Cooking_Loss ~ Treatment * Day, data = cl_long)
summary(cl_anova)
# Checking assumptions of the annova for pH
shapiro.test(residuals(cl_anova))
# residual plots ( residuals vs fitted) 
par(mfrow = c(1, 2), mar = c(4, 4, 2, 1))
qqnorm(residuals(cl_anova))
qqline(residuals(cl_anova))
plot(cl_anova, which = 1)
# Homogeneity of variance 
install.packages("car")
library(car)
leveneTest(Cooking_Loss ~ Treatment * Day, data = cl_long)
...
cld_trt <- emmeans(cl_anova, ~ Treatment | Day) |> 
...
# GT table
gt_table_cl <- final_display |> 
  gt() |> 
  tab_header(
    title = md(
      "**Table 2.** Effect of *Piper chaba* extract on cooking loss of fresh chicken patties during storage."
    )
  ) |> 
...
gtsave(gt_table_cl, filename = "CL_table.docx")
...
# Save the CL plot (tiff/pdf)
install.packages("extrafont")
library("extrafont")
ggsave(
  filename = "CL_line_plot.tiff",
  plot = P_cl_line,
  device = "tiff",
  width = 14,
  height = 10,
